 ...
 * @file    Pwm.c
 *
 * Drivers for PWM functionality with the Nucleo F446RE UCSC I/O Shield.
 * 
 * @author  jLab
 * @author  Adam Korycki
 *
...
const PWM PWM_2 = {&htim2, TIM_CHANNEL_1, 0x4};
const PWM PWM_3 = {&htim3, TIM_CHANNEL_2, 0x8};
const PWM PWM_4 = {&htim4, TIM_CHANNEL_1, 0x10};
const PWM PWM_5 = {&htim1, TIM_CHANNEL_3, 0x20};
const PWM PWM_6 = {&htim3, TIM_CHANNEL_4, 0x40};
const PWM PWM_7 = {&htim4, TIM_CHANNEL_3, 0x80};
...
char PWM_Init(void) {
    if (init_status == FALSE) { // if PWM module has not been inited
        ...
        htim2.Init.Period = 999; // deafault frequecy of 1 khz, changed by modifying ARRx register
...
char PWM_SetFrequency(unsigned int NewFrequency) {
    ...
    TIM1->ARR = TIM2->ARR = TIM3->ARR = TIM4->ARR = (uint32_t)(1000000.0/NewFrequency) - 1; // set auto-reload registers (ARR) accordingly (1 Mhz timer)
        ...
        if ((pinsAdded & mask) != 0) { // if pin i has been added, update duty cycle with new frequency
            switch(i) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    PWM_SetDutyCycle(PWM_7, duty_cycles[i]);
                    break;
            }
        }
...
char PWM_SetDutyCycle(PWM PWM_x, unsigned int Duty) {
    ...
    switch(PWM_x.mask) { // set capture compare register (CCR) to correct value and save duty cycle value
        ...
        case 0x2: // PWM_1
            ...
            break;
        case 0x4: // PWM_2
            TIM2->CCR1 = (uint32_t)((Duty/100.0)*(TIM2->ARR));
            duty_cycles[2] = Duty;
            break;
        case 0x8: // PWM_3
